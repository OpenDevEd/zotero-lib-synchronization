...
function mapEscape(character: string) {
	return alwaysMap[character as keyof typeof alwaysMap];
}

function mapAccent(character: string) {
	return (mappingTable[character as keyof typeof mappingTable] ? mappingTable[character as keyof typeof mappingTable] : "?");
}

function mapHTMLmarkup(characters: string){
...
function unescapeHTML(html: string) {
    ...
    return withoutTags.replace(/&[^;]+;/g, entity => entities[entity] || entity);
}
...
export class BibTeXExporter {
    ...
    constructor(item: any, options: any = {}) {
		...
		this.item.attachments = this.item.attachments || this.item.attachmentsData?.map((attachment: any) => ({
    ...
    writeField(field: string, value: string, isMacro: boolean = false) {
            if (!value && typeof value != "number") return;
            value = value + ""; // convert integers to strings
            this.writeLine(",\n\t" + field + " = ");
            if (!isMacro) this.writeLine("{");
            ...
            if (this.options.exportCharset != "UTF-8") {
        ...
        if(this.item.extra) {
			if (!doi) {
				...
				for (const line of split) {
					if (line.includes("DOI:")) {
						doi = line.split("DOI:")[1].trim();
					}
				}
			}
		}
    ...
    // Helper method for converting numbers to ordinal words
    private numberToOrdinal(n: number): string {
        const ordinals = [
        ...
        // For numbers > 10, just append the number with suffix
