...
export async function get_oldest_item(items: string[]): Promise<MergeItems> {
  ...
  let allItemsArray: any[] = Object.values(allItems);
...
async function mergeTwoItems(groupid: string, base: string, deleted: string): Promise<number> {
  ...
  if (baseItem.result?.data.relations) {
  ...
  try {
    ...
    if (baseItem.result?.key) {
      ...
      if (deletedItem.result?.meta?.numChildren && deletedItem.result?.meta?.numChildren > 0) {
